...
export default function SettingsPage({ onChanged, notify }) {
  ...
  if (!s) return <div className="p-8 text-sm text-neutral-400">Loading settings…</div>;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 flex flex-col gap-6 fade-up">
      ...
      {s.admin_required && (
        <section className="panel border-amber-500/40 bg-amber-500/10 p-4">
          <div className="text-sm font-semibold text-amber-200">Admin token</div>
          <p className="mt-1 text-xs text-amber-200">Shared deployment: only the owner can change settings. Paste the server's <code>FORM_ADMIN_TOKEN</code>.</p>
            ...
            <button key={id} type="button" className="provider-card" data-active={id === active} disabled={busy}
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="mt-0.5 text-xs text-neutral-400">{PROVIDER_META[id].blurb}</div>
                </div>
                {p.has_api_key ? <span className="badge bg-emerald-500/20 text-emerald-200">key {p.api_key_hint}</span> : <span className="badge bg-white/10 text-neutral-400">no key</span>}
              </div>
              <div className="mt-3 text-xs text-neutral-400">model · <span className="font-medium text-neutral-200">{p.model}</span></div>
      ...
      <section className="panel p-5 flex flex-col gap-2">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-neutral-400">Which model confirms "is this a fillable form?" for scans and ambiguous documents. Extraction still uses the active provider above.</p>
          </div>
        ...
        <div className="text-xs text-neutral-400">Effective now: <span className="font-medium text-neutral-200">{s.providers[s.gate_provider_effective]?.label}</span>{s.gate_provider !== "auto" && s.gate_provider !== s.gate_provider_effective && <span className="text-amber-300"> — {s.gate_provider} has no key, falling back</span>}</div>
      </section>
      ...
      <section className="panel p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          {pv.has_api_key && <span className="text-xs text-neutral-400">{pv.key_source === "env" ? "from server environment" : "saved on server"}</span>}
        </div>
        ...
        <div className="flex flex-col gap-2 rounded-xl p-3 sm:flex-row sm:items-center" style={{ background: "var(--surface-2)" }}>
          <label className="flex flex-1 items-center gap-2 text-sm">
            ...
            <span className={`badge ${models.live ? "bg-emerald-500/20 text-emerald-200" : "bg-white/10 text-neutral-400"}`} title={models.live ? "Listed live from your key" : "Suggestions — add a key to list your models"}>{models.live ? "live" : "suggested"}</span>
          </label>
          <div className="flex gap-2">
        ...
        <p className="text-xs text-neutral-400">If the chosen model is overloaded (503/429) the backend automatically retries on a fallback model and reports which one answered.</p>
        <p className="text-xs text-neutral-400">Keys are stored on the backend only and never returned to the browser in full. Get one at {meta.console}.</p>
      </section>

      {/* Feature toggles */}
      <section className="panel p-5 flex flex-col divide-y divide-neutral-800">
      ...
      <section className="panel p-5 flex flex-col gap-4">
        ...
        <div className="grid gap-2">
          {OCR_OPTIONS.map((o) => {
            ...
            return (
              <label key={o.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm transition ${s.ocr_backend === o.id ? "border-brand-600 bg-brand-500/15" : "border-neutral-800 hover:border-neutral-700"}`}>
                ...
                <div className="flex-1">
                  <div className="font-medium flex items-center gap-2">
                    {o.label}
                    {unavailable && <span className="badge bg-white/10 text-neutral-400">not available</span>}
                  </div>
                  <div className="text-xs text-neutral-400">{o.help}</div>
                </div>
              </label>
            );
          })}
  ...
  return (
    <div className="flex items-center justify-between gap-6 py-3 first:pt-0 last:pb-0">
      <div>
        ...
        <div className="text-xs text-neutral-400">{help}</div>
      </div>
      {children}
    </div>
  );
